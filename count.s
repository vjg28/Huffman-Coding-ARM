@@@ Count characters from STDIN
@@@ vim: set tabstop=4 softtabstop=0 expandtab shiftwidth=4 smarttab bg=dark: 
@@@ vim: ft=arm 

@@@ External Functions
    .global fputs
    .global puts
    .global itoa
    .global read
    .global check_read_error

@@@ Exported Functions
    .global count_from_file
    .global init_counts
    .global count_characters
    .global print_counts
    .global status
    .global init_sorted_chars
    .global sort_chars
    .global print_sorted
    .global print_tree_arr
    .global tree_start
    .global num_tree
    .global print_num_arr

@@@ Code Section
    
count_from_file:
    @@ Count characters from the given file handle
    @@ Arguments: R0 = File handle
    PUSH    {R0-R12,LR}         @ Push the existing registers on to the stack
    MOV     R4,R0               @ R4 = File handle
cff_loop:
    MOV     R0,R4               @ Set file handle
    LDR     R1,=buffer          @ Set buffer location
    MOV     R2,#4096            @ Set buffer size
    BL      read                @ Read bytes
    MOV     R5, R0              @ R5 = Bytes read
    CMP     R0,#0               @ Check for Errors
    BLLT    check_read_error    @ Warn about a bad address
    CMP     R0,#0               @ Check for EOF
    BGT     cff_helper          @ Count characters and loop if not EOF
    POP     {R0-R12,PC}         @ Return when loop completes, restore registers
cff_helper:
    @@ Arguments: R0 = Character count
    MOV     R1,R0               @ Move character count to R1
    LDR     R0,=buffer          @ Read from the buffer
    BL      count_characters    @ Count characters
    B       cff_loop            @ Loop 

status:
    @@ print status
    PUSH    {R0,LR}
    LDR     R0,=status_s
    BL      puts
    POP     {R0,PC}

count_characters:
    @@ Counts characters 
    @@ Arguments: 
    @@   R0 = Address of buffer
    @@   R1 = Length of buffer
    PUSH    {R0-R12,LR}         @ Push the existing registers on to the stack
    LDR     R5,=counts          @ R5 = Address of the count array
    SUBS    R2,R0,#1            @ R2 = One byte before the first memory location
count_loop:
    ADDS    R2,#1               @ R2 = Current memory location
    LDRB    R3,[R2]             @ R3 = Current byte
    LDR     R4,[R5,R3,LSL #2]   @ R4 = Current count (R5 + (R3*4))
    ADDS    R4,#1               @ Increment the count
    STR     R4,[R5,R3,LSL #2]   @ Store the count back into the array
    SUBS    R1,#1               @ Reduce loop counter by 1
    BNE     count_loop          @ Loop if counter != 0
    POP     {R0-R12,PC}         @ Return when loop completes, restore registers

sort_chars:
    @@ Copies the characters into an array sorted by occurance
    PUSH    {R0-R12,LR}         @ Push the existing registers on to the stack
    LDR     R0,=sorted_chars    @ R0 = Memory location of sorted array
    MOV     R1,#1280            @ R1 = Length of sorted array
    LDR     R2,=counts          @ R2 = Memory location of counts array
    MOV     R3,#256             @ R3 = Length of counts array
    MOV     R4,#0               @ R4 = Current sorted array index
sort_chars_next:
    MOV     R5,#0               @ R5 = Current character value
    MOV     R6,R2               @ R6 = Current memory location
    ADDS    R7,R2,R3,LSL #2     @ R7 = Ending memory location (R2 + (R3 * 4)
    MOV     R8,#0               @ R8 = Value at current memory location
    MOV     R9,#0               @ R9 = Character with max count
    MOV     R10,#0              @ R10 = Memory location of max count
    MOV     R11,#0              @ R11 = Value at max count memory location
sort_chars_loop:
    LDR     R8,[R6]             @ Load value
    CMP     R8,R11              @ Compare current value to max value
    MOVGT   R9,R5               @ If current > max, update max character
    MOVGT   R10,R6              @ If current > max, update max memory location
    MOVGT   R11,R8              @ If current > max, update max value
    ADDS    R5,#1               @ Increment character number
    ADDS    R6,#4               @ Increment the memory location
    CMP     R6,R7               @ Have we reached the end?
    BNE     sort_chars_loop     @ If not, loop again
sort_chars_loop_done:
    CMP     R10,#0              @ Did we find a max count?
    BLEQ    sort_chars_l_exit   @ If not, we're done
    STRB    R9,[R0,R4]          @ Store max char at current sorted array index
    ADDS    R4,#1               @ Increment sorted array index
    STR     R11,[R0,R4]         @ Store max count at current sorted array index + 1
    ADDS    R4,#4               @ Increment sorted array index
    MOV     R11,#0              @ Set max count to 0
    STR     R11,[R10]           @ Write 0 into the max count memory location
    CMP     R4,R3               @ Have we reached the end of the sorted array?
    BLT     sort_chars_next     @ If not, do the whole thing again
sort_chars_l_exit:
    POP     {R0-R12,PC}         @ Pop the registers off of the stack and return

init_counts:
    @@ Initialize the memory used for counting
    PUSH    {R0-R12,LR}         @ Push registers on to stack
    LDR     R0,=counts          @ Starting memory location
    MOV     R1,#1024            @ Array size
    MOV     R2,#0               @ Initial value
    BL      init_array          @ Initialize array
    POP     {R0-R12,PC}         @ Return when loop completes, restore registers

init_sorted_chars:
    @@ Initialize the memory used for sorting
    PUSH    {R0-R12,LR}         @ Push registers on to stack
    LDR     R0,=sorted_chars    @ Starting memory location
    MOV     R1,#1280            @ Array size
    MOV     R2,#0xFFFFFFFF      @ Initial value
    BL      init_array          @ Initialize array
    POP     {R0-R12,PC}         @ Return when loop completes, restore registers

init_array:
    @@ Initialize an array - Array size must be divisible by 32
    @@ Arguments: R0 = Memory location of array, R1 = Array length in bytes, R2 = Value
    PUSH    {R0-R12,LR}         @ Push registers on to stack
    ADDS    R3,R0,R1            @ R3 = Ending memory location
    MOV     R4,R0               @ R4 = Current memory location
    MOV     R5,R2               @ value to store
    MOV     R6,R2               @ value to store
    MOV     R7,R2               @ value to store
    MOV     R8,R2               @ value to store
    MOV     R9,R2               @ value to store
    MOV     R10,R2              @ value to store
    MOV     R11,R2              @ value to store
    MOV     R12,R2              @ value to store
init_array_loop:
    STMIA   R4!,{R5-R12}        @ Store 0 in the next 8 words of memory
    CMP     R4,R3               @ Have we reached the end of the array?
    BNE     init_array_loop     @ Loop again if current != end
    POP     {R0-R12,PC}         @ Return when loop completes, restore registers

print_counts:
    @@ Prints a list of counts
    PUSH    {R0-R12,LR}         @ Push the existing registers on to the stack
    BL      print_count_header  @ Print header
    MOV     R0,#0               @ Character value
    LDR     R4,=counts          @ Starting memory location
    LDR     R5,=counts_end      @ Ending memory location
print_counts_loop:
    LDR     R1,[R4],#4          @ Load count and increment memory location
    CMP     R1,#0               @ Did we have any matches?
    BLNE    print_count_line    @ Print the count line if we had a match
    ADDS    R0,R0,#1            @ Increment character number
    CMP     R4,R5               @ Have we reached the end?
    BNE     print_counts_loop   @ If not, loop again
    POP     {R0-R12,PC}         @ Pop the registers off of the stack and return

print_count_header:
    PUSH    {R0-R12,LR}         @ Push the existing registers on to the stack
    LDR     R0,=header          @ Print header
    BL      puts                @ |
    POP     {R0-R12,PC}         @ Pop the registers off of the stack and return

print_count_line:
    @@ Arguments: Character in R0, Count in R1
    PUSH    {R0-R12,LR}         @ Push the existing registers on to the stack
    MOV     R4,R0               @ R4 = Character
    MOV     R5,R1               @ R5 = Count
    LDR     R0,=line_part1      @ Print the first part of the line
    BL      fputs               @ |
    MOV     R0,R4               @ Convert character to a string
    LDR     R1,=char_string     @ | Write to the char_string memory location
    BL      itoa                @ | Get string representation
    MOV     R0,R1               @ Print the character string
    BL      fputs               @ |
    LDR     R0,=line_part2      @ Print the second part of the line
    BL      fputs               @ |
    MOV     R0,R5               @ Convert count to a string
    LDR     R1,=count_string    @ | Write to the count_string memory location
    BL      itoa                @ | Get string representation
    MOV     R0,R1               @ Print the count string
    BL      puts                @ |
    POP     {R0-R12,PC}         @ Pop the registers off of the stack and return

print_sorted:
    @@ Prints a list of counts
    PUSH    {R0-R12,LR}         @ Push the existing registers on to the stack
    BL      print_sorted_header @ Print header
    LDR     R4,=sorted_chars    @ Starting memory location
    ADDS    R5,R4,#1280         @ Ending memory location

print_sorted_loop:
    LDRB    R0,[R4],#1          @ Load character and increment memory location
    LDR     R1,[R4],#4          @ Load count and increment memory location
    CMP     R0,#0xFF            @ Is this value being used?
    BLNE    print_count_line    @ If so, print the count line
    CMP     R4,R5               @ Have we reached the end?
    BNE     print_sorted_loop   @ If not, loop again
    POP     {R0-R12,PC}         @ Pop the registers off of the stack and return

print_sorted_header:
    PUSH    {R0-R12,LR}         @ Push the existing registers on to the stack
    LDR     R0,=sorted_header   @ Print header
    BL      puts                @ |
    POP     {R0-R12,PC}         @ Pop the registers off of the stack and return

print_checker:
    PUSH    {R0-R12,LR}         @ Push the existing registers on to the stack
    LDR     R0,=checker         @ Print header
    BL      puts                @ |
    POP     {R0-R12,PC}         @ Pop the registers off of the stack and return



@@@ My functions

tree_start:
    PUSH    {R0-R12,LR}         @ Push the existing registers on to the stack
    BL      print_tree_header   @ Prints a header indicating starting of tree
    BL      init_tree_chars     @ Initialize new array for tree
    LDR     R0, =sorted_chars   @ Starting sorted array Memory Location
    ADDS    R1, R0, #1280       @ End sorted arr memory location
    MOV     R3, R0              @ Copy sorted array staring address into R3
    ADDS    R3,R3,#5            @ For comparision purposes
    LDR     R4,=tree_array      @ Starting tree array memory location
    ADDS    R5, R4, #2560       @ End tree array location
    MOV     R11,#0xFE           @ Special Character          
    
reverse_sort:
    @@ Gives the address of the last element in the sorted array
    @@ Input R0: array starting address, R1: array last address
    LDRB    R2,[R0]             @ Load character into R2
    CMP     R2,#0xFF            @ Is this value being used?
    BEQ     tree_loop           @ If not, then return to tree code
    ADDS    R0,R0,#5            @ Increment the memory location by 5
    CMP     R0,R1               @ Have we reached the end?
    BNE     reverse_sort        @ If not, loop again

tree_loop:
    @@ Input:: R0: last element address(SA), R3: start memory location(SA)
    @@ Input:: R4: tree starting address(TA),R5: tree end location(TA)
    MOV     R2,#0xFFFFFFFF      @ Clearing sorting array with this
    SUBS    R0,R0,#4            @ Subtracting end address
    LDR     R6,[R0]             @ Loading 1st counter value into R6
    STR     R2,[R0]             @ Clearing the value
    SUBS    R0,R0,#1            @ Subtracting end address
    LDRB    R7,[R0]             @ Loading 1st character value into R7
    STRB    R2,[R0]             @ Clearing the byte value
    SUBS    R0,R0,#4            @ Subtracting end address
    LDR     R8,[R0]             @ Loading 2nd counter value into R8
    STR     R2,[R0]             @ Clearing the value
    SUBS    R0,R0,#1            @ Subtracting end address
    LDRB    R9,[R0]             @ Loading 2nd character value into R9
    STRB    R2,[R0]             @ Clearing the byte value
    ADDS    R10,R8,R6           @ Summed up frequency
    STRB    R11,[R0],#1         @ Inserting Node element back into array and incrementing by 1
    STR     R10,[R0],#4         @ Inserting node frequency and back to old array
    STRB    R7,[R4],#1          @ Store small char into tree array and increment address
    STR     R6,[R4],#4          @ Store small char frequency into tree arr and increment address
    STRB    R9,[R4],#1          @ Store small char into tree array and increment address
    STR     R8,[R4],#4          @ Store small char frequency into tree arr and increment address
    CMP     R3,R0               @ Is there only one element in the sorted list??
    BEQ     tree_exit           @ If so, then exit the loop                
    BL      sort_tree           @ Sort the old array with an extra node element
    @BL      print_sorted        @ Print the sorted array
    B       tree_loop           @ All way back to the top
    
tree_exit:
    SUBS    R0,R0,#4            @ Subtracting address
    LDR     R6,[R0]             @ Loading 1st freq value into R6
    STR     R2,[R0]             @ Clearing the value
    SUBS    R0,R0,#1            @ Subtracting end address
    LDRB    R7,[R0]             @ Loading 1st character value into R7
    STRB    R2,[R0]             @ Clearing the byte value
    STRB    R7,[R4],#1          @ Store small char into tree array and increment address
    STR     R6,[R4],#4          @ Store small char frequency into tree arr and increment address
    LDR     R0,=tree_done       @ Indication of tree loop completion
    BL      puts                @ Print tree done
    POP     {R0-R12,PC}         @ Pop the registers off of the stack and return
    


num_tree:
    @@ Function to get tree index   
    PUSH    {R0-R12,LR}         @ Push the existing registers on to the stack
    LDR     R0,=tree_indexing   @ Print header
    BL      puts                @ print
    BL      init_num_array      @ Initialize new array for saving indexes
    LDR     R0, =tree_array     @ R0 = Last tree array element Pointer
    LDR     R2, =num_array      @ R2 = Starting num array Memory Location
    ADDS    R1, R0, #2560       @ R1 = End tree arr memory location
    MOV     R3,R0               @ R3 = Starting tree array address
    MOV     R4,R2               @ R4 = Filled num array pointer  
reverse:
    @@ Gives the address of the last element in the sorted array
    @@ Input R0: array starting address, R1: array last address
    LDRB    R11,[R0]            @ Load character into R11
    CMP     R11,#0xFF           @ Is this value being used?
    BEQ     num_cont            @ If not, then return to num code
    ADDS    R0,R0,#5            @ Increment the memory location by 5
    CMP     R0,R1               @ Have we reached the end?
    BNE     reverse             @ If not, loop again  
num_cont:
    MOV     R11,#1              @ Saving 1 in R11
    STR     R11,[R4,#1]         @ Saving 1 in the first index (default)
    ADDS    R4,R4,#5            @ R4 pointing to second element
    MOV     R5,#0               @ Loop Counter
    MOV     R12,#0xFE           @ Special char
num_loop:
    ADDS    R5,R5,#1            @ Incrementing counter
    SUBS    R0,R0,#5            @ tree array pointer
    LDRB    R6,[R0]             @ Load character
    LSL     R7,R5,#2            @ R7 = counter no * 4
    ADDS    R7,R7,R5            @ R7 = counter no * 4 + counter_no 
    SUBS    R7,R7,#5            @ R7 = counter no * 4 - 5 + counter_no
    ADDS    R7,R7,R2            @ R7 = pointer in num_array
    STRB    R6,[R7]             @ Insert character in num_array
    CMP     R0,R3               @ Have we completed readind array??
    BEQ     num_exit            @ If so, out you go
    CMP     R6,R12              @ Is character == special character??
    BNE     num_loop            @ If not equal, then up you go
    LDR     R8,[R7,#1]          @ R8 = num_array[counter-1] 
    LSL     R9,R8,#1            @ R9 = 2*R8
    ADDS    R10,R9,#1           @ R10 = 2*R8+1
    STR     R9,[R4,#1]          @ Save in num_array[R4_pointer]
    ADDS    R4,R4,#5            @ increment pointer
    STR     R10,[R4,#1]         @ Save in num_array[R4_pointer+1]
    ADDS    R4,R4,#5            @ increment pointer
    B       num_loop            @ Back to where we started
num_exit:
    LDR     R0,=completed_num   @ Print header
    BL      puts                @ print
    POP     {R0-R12,PC}         @ Pop the registers off of the stack and return
    


print_tree_header:
    PUSH    {R0-R12,LR}         @ Push the existing registers on to the stack
    LDR     R0,=tree_header     @ Print header
    BL      puts                @ |
    POP     {R0-R12,PC}         @ Pop the registers off of the stack and return



init_tree_chars:
    @@ Initialize the memory used for tree
    PUSH    {R0-R12,LR}         @ Push registers on to stack
    LDR     R0,=tree_array      @ Starting memory location
    MOV     R1,#2560            @ Array size
    MOV     R2,#0xFFFFFFFF      @ Initial value
    BL      init_array          @ Initialize array
    POP     {R0-R12,PC}         @ Return when loop completes, restore registers   
    


init_num_array:
    @@ Initialize the memory used for tree
    PUSH    {R0-R12,LR}         @ Push registers on to stack
    LDR     R0,=num_array      @ Starting memory location
    MOV     R1,#2560            @ Array size
    MOV     R2,#0xFFFFFFFF      @ Initial value
    BL      init_array          @ Initialize array
    POP     {R0-R12,PC}         @ Return when loop completes, restore registers  




sort_tree:
    @@ Array sorted by frequency (Bubble Sort)
    PUSH    {R0-R12,LR}         @ Push the existing registers on to the stack
    MOV     R3,R0               @ R3 = Last element memory address
    LDR     R0,=sorted_chars    @ R0 = Memory location of array
    ADDS    R1,R0,#1280         @ R1 = End memory address
sort_tree_initialize:
    SUBS    R3,R3,#4            @ |
    LDR     R5,[R3]             @ Loading last element frequency
    SUBS    R3,R3,#1            @ |
    LDRB    R4,[R3]             @ Loading last element character
sort_tree_loop:
    SUBS    R3,R3,#4            @ loading above element
    LDR     R7,[R3]             @ |
    SUBS    R3,R3,#1            @ loading above char
    LDRB    R6,[R3]             @ |
    CMP     R5,R7               @ Comparing frequencies
    BLT     sort_tree_exit      @ If R5 < R7 then break 
    STRB    R4,[R3]             @ Swap places
    STR     R5,[R3,#1]          @ |
    STRB    R6,[R3,#5]          @ |
    STR     R7,[R3,#6]          @ |
    CMP     R0,R3               @ Compare if we have reached to the first index
    BEQ     sort_tree_exit      @ If so, then out you go
    B       sort_tree_loop      @ Jump to loop again till you break by any way  
sort_tree_exit:
    @LDR     R0,=sort_done       @ Indication of loop completion
    @BL      puts                @ Print sort done
    POP     {R0-R12,PC}         @ Pop the registers off of the stack and return
    
    


print_tree_arr:
    @@ Prints a list of TREE array
    PUSH    {R0-R12,LR}         @ Push the existing registers on to the stack
    BL      print_tree_line     @ Print header
    LDR     R4,=tree_array      @ Starting memory location
    ADDS    R5,R4,#2560         @ Ending memory location
print_tree_loop:
    LDRB    R0,[R4],#1          @ Load character and increment memory location
    LDR     R1,[R4],#4          @ Load count and increment memory location
    CMP     R0,#0xFF            @ Is this value being used?
    BLNE    print_count_line    @ If so, print the count line
    CMP     R4,R5               @ Have we reached the end?
    BNE     print_tree_loop     @ If not, loop again
    POP     {R0-R12,PC}         @ Pop the registers off of the stack and return
print_tree_line:
    PUSH    {R0-R12,LR}         @ Push the existing registers on to the stack
    LDR     R0,=tree_line       @ Print header
    BL      puts                @ |
    POP     {R0-R12,PC}         @ Pop the registers off of the stack and return
    

    
print_num_arr:
    @@ Prints a list of NUM array
    PUSH    {R0-R12,LR}         @ Push the existing registers on to the stack
    BL      print_num_line      @ Print header
    LDR     R4,=num_array      @ Starting memory location
    ADDS    R5,R4,#2560         @ Ending memory location
print_num_loop:
    LDRB    R0,[R4],#1          @ Load character and increment memory location
    LDR     R1,[R4],#4          @ Load count and increment memory location
    CMP     R0,#0xFD            @ Is this value being used?
    BLLE    single_num_line     @ If so, print the count line
    CMP     R4,R5               @ Have we reached the end?
    BNE     print_num_loop      @ If not, loop again
    POP     {R0-R12,PC}         @ Pop the registers off of the stack and return
print_num_line:
    PUSH    {R0-R12,LR}         @ Push the existing registers on to the stack
    LDR     R0,=num_line        @ Print header
    BL      puts                @ |
    POP     {R0-R12,PC}         @ Pop the registers off of the stack and return



single_num_line:
    @@ Arguments: Character in R0, Count in R1
    PUSH    {R0-R12,LR}         @ Push the existing registers on to the stack
    MOV     R4,R0               @ R4 = Character
    MOV     R5,R1               @ R5 = Count
    LDR     R0,=num_part1       @ Print the first part of the line
    BL      fputs               @ |
    MOV     R0,R4               @ Convert character to a string
    LDR     R1,=char_string     @ | Write to the char_string memory location
    BL      itoa                @ | Get string representation
    MOV     R0,R1               @ Print the character string
    BL      fputs               @ |
    LDR     R0,=num_part2       @ Print the second part of the line
    BL      fputs               @ |
    MOV     R0,R5               @ Convert count to a string
    LDR     R1,=count_string    @ | Write to the count_string memory location
    BL      itoa                @ | Get string representation
    MOV     R0,R1               @ Print the count string
    BL      puts                @ |
    LDR     R0,=num_part3       @ Print the third part of the line
    BL      fputs               @ |
    MOV     R0,R5               @ Required
    BL      print_r0_binary     @ Print addressing
    POP     {R0-R12,PC}         @ Pop the registers off of the stack and return
    
        
print_val:
    @@ For printing register byte value (Put your register in place of R11)
    PUSH    {R0-R12,LR}         @ Push registers on to stack
    MOV     R0,R11              @ Convert character to a string (Set 
    LDR     R1,=char_string     @ | Write to the char_string memory location
    BL      itoa                @ | Get string representation
    MOV     R0,R1               @ Print the character string
    BL      fputs               @ |
    POP     {R0-R12,PC}         @ Pop the registers off of the stack and return

@@@ Data Section
    
    .data

counts: .space 1024
counts_end: .word 0
@@ Each element in the sorted character array is 5 bytes: a 1 byte char and a 4 byte count 
sorted_chars: .space 1280
reverse_sorted_chars: .space 1280
tree_array: .space 2560        
@@ Each element of the tree array will be 5 bytes and 511 elements(at max) [ 2*n -1 ]
num_array: .space 2560
char: .word 0
buffer: .space 4096
tree_buffer: .space 4
count_string: .asciz "0000000000" @ max 4294967296
char_string: .asciz "000" @ max 256
status_s: .asciz "Counting Characters"
header: .asciz "Character Counts:"
line_part1: .asciz "\tCharacter: "
line_part2: .asciz "\tCount: "
sorted_header: .asciz "Sorted Characters:"
tree_header: .asciz "Starting Tree construction:-------------"
sort_done:  .asciz "Eh! Sort done!!"
tree_done:  .asciz "Tree array made:---------------------"
tree_line: .asciz "Printing Tree Array:"
num_line: .asciz "Printing Num array:"
checker: .asciz "Checked!!"
tree_indexing: .asciz "Starting Tree Indexing!"
completed_num: .asciz "Indexing complete"
num_part1: .asciz "\tCharacter: "
num_part2: .asciz "\tAddressing: "
num_part3: .asciz "\tEncoded: "
